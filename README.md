...
- 将鉴权、只读安全网关、请求执行、员工/单据脱敏与摘要逻辑沉淀到同级独立目录 `../hesi-openapi-core/`，便于开源版和商业 BI Agent 共同维护。
...
- 自动获取并缓存 `accessToken`
- 通过共享 core 的 `client.cjs` 直连合思 API，`expense_api.cjs` 保持为稳定 CLI 入口
...
```text
../hesi-openapi-core/
  client.cjs          # 鉴权、token 缓存、只读 API 请求执行
  safety.cjs          # 同源域名、HTTP 方法、危险接口拦截
  expense-docs.cjs    # 员工匹配、员工/单据脱敏、单据汇总
  errors.cjs          # 统一错误码
  index.cjs           # 核心导出入口

vendor/hesi-openapi-core/
  # 发布前由 scripts/sync-core.cjs 从 ../hesi-openapi-core/ 同步生成

core_loader.cjs       # 开发期加载 ../hesi-openapi-core，发布包内加载 vendor/hesi-openapi-core
expense_api.cjs       # 开源版底层 API CLI，调用共享核心
expense_query.cjs     # 开源版本人业务查询 CLI，调用共享核心
expense_bi_demo.cjs   # 本地 BI 演示，不调用真实合思接口
```

后续收费版/商业 BI Agent 应优先复用 `../hesi-openapi-core/`。单独发布某个 skill 前，运行：

```bash
node scripts/sync-core.cjs
```

这会把独立 core 同步到当前 skill 的 `vendor/hesi-openapi-core/`，保证单独安装时也能运行。
...
商业版应在独立目录中新增角色权限、真实企业级 BI、客户字段映射和报表模板，不要把客户定制逻辑写进开源入口。
...
项目内置 mock 测试，不依赖真实合思凭证或网络。请先确认当前 `node` 是 18 或更高版本；仓库 `.nvmrc` 指定了 `v21.6.1`。

```bash
node tests/run-mock-tests.cjs
```

测试覆盖共享核心加载、本地预检、安全拦截、本人单据查询、本人单据汇总和越权拒绝。

## 从开源版到企业版

开源版适合验证“能不能安全接入”。生产可用的企业财务 Agent，通常还需要按企业实际环境定制：

- 复用 `../hesi-openapi-core/`，保持鉴权、安全拦截和脱敏口径一致
- 私有报销表单字段映射
- 一句话自动创建报销单、差旅单、付款单
- 费用标准校验和超标拦截
- 预算查询、预算占用和项目成本中心联动
